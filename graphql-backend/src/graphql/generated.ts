...
const ResultModel = mongoose.model<ResultMongoose>('Result', ResultSchema);
export { ResultModel };
...
export interface ResultMongoose {
  id: Maybe<number>;
  result: Maybe<string>;
}
const AuthenticateResponseSchema = new Schema({
  token: { type: String, required: true }
});


export interface AuthenticateResponseInput {
  token: string;
}

export interface AuthenticateResponseMongoose {
  token: string;
}
